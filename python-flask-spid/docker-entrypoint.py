...
def mkcert(key_path, cert_path, hostname, ou):
    ssl_command = (r'openssl req -x509 -nodes -sha256'
                   r' -subj "/C=IT/ST=Lazio/O=MyCompany/OU={ou}/CN={hostname}"'
                   r' -newkey rsa:2048 -keyout {key_path} -out {cert_path}')

    safe_system(ssl_command.format(key_path=key_path,
                                   cert_path=cert_path, hostname=hostname, ou='IDP'))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--dummy-config', dest='dummy_config', action="store_true",
    ...
    if args.dummy_config:
        dummy_config = {
            'base_url': None,
            'debug': True,
            'endpoints': {'single_logout_service': '/slo',
        ...
        if not exists(dummy_config['cert_file']) and not exists(dummy_config['key_file']):
            mkcert(key_path=dummy_config['key_file'], cert_path=dummy_config[
                   'cert_file'], hostname='localhost', ou='IDP')

        if not exists(dummy_config['https_cert_file']) and not exists(dummy_config['https_key_file']):
            mkcert(key_path=dummy_config['https_key_file'], cert_path=dummy_config[
                   'https_cert_file'], hostname=hostname, ou='Website')

        if args.insecure_sp:
            metadata_path = pjoin(
                basedir, "_" + args.insecure_sp.replace("/", "_") + ".xml")
            safe_system("curl -k {metadata_url} -o {metadata_path}".format(
                metadata_path=metadata_path,
                metadata_url=args.insecure_sp
            ))
